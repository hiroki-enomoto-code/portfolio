...
import BeatPost from '@/components/templetes/beat/BeatPost/BeatPost';
...
export default function Page() {
    ...
    const beatApi = useQueryApi<BeatListProps>({ queryKey: ['beat'], url: `${API_PATH}/beat?${new URLSearchParams(args.current as any)}` });

    const [refetching, setRefetching] = React.useState(false);

    const handlePost = (item : BeatProps) => {
    ...
    const handleMoreBeat = async() => {
        ...
        setRefetching(true);
        await axios.get<BeatListProps>(`${API_PATH}/beat?${new URLSearchParams(args.current as any)}`)
            .then((res) => {
    ...
    return (
        <div id="page-beat" className="page-beat">
            ...
            { beatApi.data?.data && <div ref={ sensorRef } className="page-beat_sensor"/> }
        </div>
    );
}